...
import pl.polsl.wylegly_machulik.mal.data.SensorReading;
...
public class PageSupplier
{
    ...
    private Aggregate createAggregate(List<SensorReading> sensorReadings, LocalDateTime startTimestamp)
    {
     ...
     //   LOG.info("Create agrregate  startTimestamp={}, thread= {}", startTimestamp, Thread.currentThread().getId());
        return Aggregate.builder().waterLevelReadings(waterLevelReadings).startTimestamp(startTimestamp).endTimestamp(
                startTimestamp.plusMinutes(aggregationWindowWidthMinutes)).isAllReadyRead(false).build();
    }
